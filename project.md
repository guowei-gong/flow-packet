...
### 3.3 UI 设计风格：UE5 蓝图编辑器

flow-packet 的界面参考 Unreal Engine 5 蓝图编辑器的视觉语言和布局逻辑，为游戏开发者提供熟悉的操作体验。

#### 3.3.1 整体布局

```
┌──────────────────────────────────────────────────────────────────────────┐
│  菜单栏   [Flow Packet]  [文件]  [编辑]  [视图]  [运行]                      │
├──────────────────────────────────────────────────────────────────────────┤
│  工具栏   [▶ 运行] [⏹ 停止] [⏸ 暂停]  ──  🟢 TCP 已连接 127.0.0.1:9001   │
├────────────────┬─────────────────────────────────────┬───────────────────┤
│                │                                     │                   │
│   协议浏览器    │           画布区域                    │   属性面板         │
│   (左侧面板)    │      (中央，占据主要空间)              │   (右侧面板)       │
│                │                                     │                   │
│  ▼ game.proto  │    ┌──────────┐    ┌──────────┐     │  ▌连接配置         │
│    ▶ LoginReq  │    │ LoginReq │───▶│ EnterRoom│     │   协议: TCP       │
│    ▶ LoginResp │    │          │    │          │     │   地址: 127.0.0.1 │
│    ▶ EnterRoom │    └──────────┘    └──────────┘     │   端口: 9001      │
│  ▼ battle.proto│                         │           │                   │
│    ▶ ReadyReq  │                    ┌────▼─────┐     │  ──────────────   │
│    ▶ StartBattle│                   │ ReadyReq │     │  ▌节点属性        │
│                │                    │          │     │   (选中节点时显示)  │
│                │                    └──────────┘     │   字段编辑器       │
│                │                                     │                   │
├────────────────┴─────────────────────────────────────┴───────────────────┤
...
│   执行日志 / 响应面板 (底部面板，可折叠)                                      │
│                                                                          │
│   [14:23:01] ▶ LoginReq  → 发送成功 (32 bytes)                            │
│   [14:23:01] ◀ LoginResp ← 收到响应 (128 bytes, 23ms)  {token: "abc..."}  │
│   [14:23:02] ▶ EnterRoom → 发送成功 (16 bytes)                            │
│                                                                          │
└──────────────────────────────────────────────────────────────────────────┘
```
...
| 元素 | 色值 | 说明 |
|------|------|------|
| 画布背景 | `#1A1A2E` | 深蓝灰，带网格线 `#222240` |
| 面板背景 | `#16162A` | 比画布更深一级 |
| 节点背景 | `#2A2A4A` | 半透明毛玻璃质感 |
| 节点标题栏 | 按类型着色 | 请求节点 `#1B6EF3`，条件 `#2ECC71`，延时 `#F39C12` |
| 连线 | `#4A90D9` | 带流动动画（执行时高亮） |
| 连线 (执行中) | `#00FF88` | 荧光绿，脉冲动画表示数据流动 |
| 引脚 (Pin) | 按数据类型着色 | string `#FF6B9D`，int `#7ED6DF`，bool `#E74C3C`，message `#F39C12` |
| 文字 | `#E0E0FF` | 主文字，高对比 |
| 文字 (次要) | `#8888AA` | 辅助信息 |
...
```
   ...
   │  ○ Exec ──────────────────────── Exec ○        │  ← 执行流引脚 (白色)
   │                                                │
   │  account   [string]  "player001"    ○ resp ──  │  ← 输出引脚
   │  password  [string]  "******"       ○ token    │
   │  platform  [enum  ]  ▼ ANDROID      ○ roomId   │
   │                                                │
   ├────────────────────────────────────────────────┤
...
核心设计特征：
- **引脚 (Pin) 系统**：左侧为输入引脚（执行流入口 + 参数绑定入口），右侧为输出引脚（执行流出口 + 响应字段输出）
- **类型着色连线**：不同数据类型的连线颜色不同，与 UE5 蓝图一致
- **执行流 vs 数据流**：白色连线表示执行顺序，彩色连线表示数据传递
- **毛玻璃节点**：半透明背景 + 微妙阴影，层次分明
- **连线动画**：执行时连线上有粒子/脉冲流动效果

#### 3.3.3 面板说明

| 面板 | 位置 | 功能 | 默认宽度 |
|------|------|------|---------|
| **协议浏览器** | 左侧 | 树状展示已导入的 Proto 文件和 message，可拖拽到画布 | 260px |
| **画布** | 中央 | 核心编排区域，节点拖拽、连线、缩放、平移 | 自适应 |
| **属性面板** | 右侧 | 连接配置（无选中时）/ 节点字段编辑器（选中节点时）| 320px |
| **执行面板** | 底部 | 运行日志、请求/响应详情、性能计时 | 200px（可折叠）|
| **工具栏** | 顶部 | 运行控制、连接状态指示、全局操作 | 固定 |

所有面板支持拖拽调整大小、折叠、浮动，与 UE5 编辑器的面板行为一致。
